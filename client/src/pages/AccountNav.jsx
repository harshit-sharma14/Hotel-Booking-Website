...
export const AccountNav = () => {
  ...
  const subpage = path.split('/')[2] || 'profile';

  const [redirect, setRedirect] = useState(null);

  function linkClass(type = null) {
    let class_new = 'inline-flex gap-1 py-2 px-6 ';
    if (type === subpage) {
      class_new += 'bg-rose-600 text-white rounded-full';
    } else {
      class_new += 'bg-gray-200 rounded-full';
    }
    return class_new;
  }
